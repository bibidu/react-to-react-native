...
import './index.scss'
import { getDateMappingWeek, getMonthDayCount } from './utils.js'
    ...
    return (
      <div className="calendar-container">
        <div className="calendar-top">
          <div className="left-btn" onClick={() => this.toggleCalendarMonth(-1)} rn-text>{'上月'}</div>
          <div className="title" rn-text>{calendarYear + '.' + ('0' + calendarMonth).slice(-2) }</div>
          <div className="right-btn" onClick={() => this.toggleCalendarMonth(1)} rn-text>{'下月'}</div>
        </div>
